<template>
  <v-card v-if="store.getPlayerState && store.getSong" width="100vw" height="10vh" flat>
    <v-slider class="mt-1 mx-2" :model-value="store.getCurrentTime" :max="store.getSong?.duration"  absolute hide-details hide-spin-buttons density="compact" @update:modelValue="updated"/>
    <v-row class="mt-2" align="center" justify="start">
      ...
      <v-spacer/>
      <v-col cols="2">
        <v-slider   :model-value="store.getVolume" :max="1"  absolute hide-details hide-spin-buttons density="compact" @update:modelValue="updatedVolume"/>
      </v-col>
      <v-col cols="auto">

        ...
        <v-data-table-server v-model:items-per-page="itemsPerPage"
          ...
          <template v-slot:[`item.edit-actions`]="{item}">
            <v-dialog v-model="dialog" max-height="60vh" max-width="30vw">
              ...
              <v-card v-if="album" title="Edit">
                <v-card-text>
                  <v-row dense>
                    <v-col cols="12">
                      <v-text-field v-model="album.title" label="Title" tile flat/>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col cols="12">
                      <v-file-input
                        v-model="album.logoFile"
                        accept="image/jpg"
                        label="Logo"
                        show-size
                        placeholder="Pick an Album logo"
                  ...
                  <v-row v-if="mode === 'create'" dense>
                    <v-col cols="12">
                      <v-autocomplete v-model="album.musicianId" :items="artists" item-title="title" item-value="id"
                                      label="Artist" tile flat/>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col cols="12">
                      <v-combobox v-model="album.genres" label="Genres" tile flat multiple/>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col cols="12">
                      <v-checkbox v-model="album.ep" label="EP" tile flat/>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col cols="12">
                      <v-textarea v-model="album.shortDescription" label="Short Description" tile flat/>
                    </v-col>
                  </v-row>
                  <v-row dense>
                    <v-col cols="12">
                      <v-text-field v-model="album.year" type="number" label="Year" tile flat/>
                    </v-col>
                  </v-row>
                </v-card-text>
                <v-card-actions>
...
async function loadItems() {
  ...
  albumRepository.get(trimmedSearch != null && trimmedSearch.length > 0 ? `title=ilike='${trimmedSearch}'` : null, {
...
async function initEdit(item) {
  mode.value = 'edit'
  album.value = Object.assign({}, item)
}

async function save() {
  if (mode.value === 'edit') {
    albumRepository.patch(album.value, album.value.id).then(() => {
      if (album.value?.logoFile) {
        albumRepository.uploadLogo(album.value.logoFile, album.value.id).then(() => {
          loadItems()
          close()
        })
      } else {
    ...
    albumRepository.create(album.value).then((response) => {
      if (album.value?.logoFile) {
        albumRepository.uploadLogo(album.value.logoFile, response.data.id).then(() => {
          loadItems()
          close()
        })
      } else {
...
function generateFileSrc(id) {
  return albumRepository.getLogoUrl(id)
}

async function deleteArtist(id) {
  albumRepository.deleteById(id).then(() => {
    loadItems()
  })
}
...
watch(dialog, (value) => {
  if (value) {
    artistRepository.getShort().then((response) => {
